...
canc⊥5 : {A : Set} {x y z : A} 
  → x ∷ [] ≡ y ∷ z ∷ [] → ⊥
canc⊥5 ()

-- canc⊥6 : {A : Set} {x y z : A} {ys : List A}
--   → (eq : x ∷ [] ≡ ys ++ y ∷ z ∷ []) → ⊥
-- canc⊥6 eq = {!   !}
...
snoc≡ : {A : Set} (xs ys : List A) {x y : A}
  → xs ++ x ∷ [] ≡ ys ++ y ∷ [] → xs ≡ ys × x ≡ y
snoc≡ [] [] refl = refl , refl
snoc≡ [] (x ∷ ys) eq with inj∷ eq
snoc≡ [] (x ∷ []) eq | refl , ()
snoc≡ [] (x ∷ x₁ ∷ ys) eq | refl , ()
snoc≡ (x ∷ xs) [] eq with inj∷ eq
snoc≡ (x ∷ []) [] eq | refl , ()
snoc≡ (x ∷ x₁ ∷ xs) [] eq | refl , ()
snoc≡ (x ∷ xs) (x₁ ∷ ys) eq with inj∷ eq
... | refl , q = cong (x ∷_) (snoc≡ xs ys q .proj₁) , snoc≡ xs ys q .proj₂

snoc≡-refl : {A : Set} (xs : List A) (x : A)
  → snoc≡ xs xs {x} {x} refl ≡ (refl , refl)
snoc≡-refl [] x = refl
snoc≡-refl (x₁ ∷ xs) x rewrite snoc≡-refl xs x = refl